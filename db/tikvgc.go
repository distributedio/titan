...
func StartTikvGC(db *DB, tikvCfg *conf.TikvGC) {
	...
	for range ticker.C {
		isLeader, err := isLeader(db, sysTikvGCLeader, id, tikvCfg.LeaseFlushInterval)
		if err != nil {
		...
		if err := runTikvGC(ctx, db, tikvCfg.SafePointLifeTime, tikvCfg.Concurrency); err != nil {
...
func runTikvGC(ctx context.Context, db *DB, uuid []byte, lifeTime time.Duration, concurrency int) error {
	...
	if lastPoint != nil && newPoint.Before(*lastPoint) {
		zap.L().Info("[TikvGC] last safe point  is later than current on ,no need to gc.", zap.Time("last", *lastPoint), zap.Time("current", *newPoint))
		return nil
	}
	...
	if err := gcworker.RunGCJob(ctx, db.kv, safePoint, UUIDString(uuid), concurrency); err != nil {
		return err
	}
	return nil
